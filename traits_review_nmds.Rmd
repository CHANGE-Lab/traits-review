...
review_traits_use = review_traits_use %>%
  select(DOI:ncol(review_traits_use))
...
review_traits_2use = review_traits_2use %>%
  select(DOI:ncol(review_traits_2use))
review_sites_2use <- as.data.frame(review_traits_2use[,1:10])
review_species_2use <- review_traits_2use[,11:ncol(review_traits_2use)] #1410 trait columns

head(review_traits_2use)
```
...
review_traits_1use = review_traits_1use %>%
  select(DOI:ncol(review_traits_1use))
...
traits_ord_2use <- metaMDS(review_species_2use,
                          distance = "jaccard",
                          trymax = 1000,
                          k = 3) #increase trymax = 50
...
#traits_ord_use <- readRDS("traits_ord_1use.rds")
