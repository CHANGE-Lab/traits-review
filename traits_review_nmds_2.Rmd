...
Note to vary the df you want to upload but to also rename any output so you don't write over previous figures or ordinations

```{r UPLOAD CLEANED DATA HERE & USE THIS DATA}
#Reload cleaned data here
#review_traits_2use <- read_csv("review_traits_2use6.csv")

review_traits_2 = read_csv("review_traits_2use30.csv") %>%
  select(DOI:ncol(.))

#USE THESE SUBSETS

review_sites_2nd <- as.data.frame(review_traits_2[,1:10]) #140 observations & 10 co-variates 
review_species_2nd <- review_traits_2[,11:ncol(review_traits_2)] #7 trait columns 140 obs observations
...
traits_ord_2 <- metaMDS(review_species_2nd,
                          distance = "jaccard",
...
#When using the _2use9 dataset we get the following error message:
#stress is (nearly) zero: you may have insufficient data

plot(traits_ord_2) #plots species (here traits) as black dots, sites (here studies) as red crosses, not labelled

#Plot sites (row numbers) and species (traits) as labels
ordiplot(traits_ord_2, type = "none") #creates blank plot
orditorp(traits_ord_2, display = "species", col = "black", air=0.01) #labels traits
orditorp(traits_ord_2, display = "sites", col = "red", air=0.01) #labels traits
...
Note that I ran the ordination for not-filled in data and it was worse than the 2nd classification, so I'm not bothering to include the code here right. Can include upon request.
